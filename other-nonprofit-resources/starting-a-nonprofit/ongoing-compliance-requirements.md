---
description: >-
  What US 501(c)(3) nonprofits must do every year to stay in good standing.
icon: clipboard-check
---

# Ongoing Compliance Requirements

Forming your nonprofit is the beginning, not the finish line. If you haven't formed your nonprofit yet, start with [Starting a Nonprofit 501(c)(3)](starting-a-nonprofit-501c3.md). Below is what you need to do every year to keep your status in good standing.

---

## Annual IRS Filing (Form 990)

All 501(c)(3) organizations must file with the IRS each year. Which form you file depends on your revenue:

| Annual Gross Receipts | Form Required |
|---|---|
| Under $50,000 | **Form 990-N** (e-Postcard, filed online, free) |
| $50,000 to $200,000 | **Form 990-EZ** |
| Over $200,000, or total assets over $500,000 | **Form 990** (full form) |

**Filing deadline:** The 15th day of the 5th month after your fiscal year ends (May 15 for a December 31 fiscal year end). Extensions are available.

**Critical:** If you miss three consecutive years of 990 filings, the IRS automatically revokes your tax-exempt status. Reinstatement is possible but costly and time-consuming.

### How to File Form 990-N (e-Postcard)

> **Note:** The steps below apply **only to Form 990-N**. If your organization files Form 990-EZ or the full Form 990, these steps don't apply. Work with your accountant or tax preparer on those forms instead.

1. Go to the IRS e-Postcard site: [https://sa.www4.irs.gov/epostcard/secure/990n/forms/list/](https://sa.www4.irs.gov/epostcard/secure/990n/forms/list/)
2. Log in (or create an account) and select your organization.
3. Start a new filing for the current year.
4. Confirm that your gross receipts were under $50,000 for the year, and confirm your organization has not been terminated or gone out of business.
5. Submit the filing.
6. Save a copy of your confirmation receipt (print to PDF) for your records.

#### Known Issue: Your Filing May Show as "Pending"

After you submit, the e-Postcard portal often shows your filing as **"Pending"**, even when it was actually accepted right away. This is a known bug in the IRS portal, not a problem with your filing. If you see "Pending," no action is required, and your filing is likely already accepted.

There is a way to correct the status in the portal, but the window to do it is short:

1. Right after you submit your 990-N, **stay on the page.**
2. Within about **7 minutes** of submitting, click the **"Get Updated Status"** button in the portal.
3. This pulls the correct, accepted status into the portal.

Because this window is so short, it's worth setting aside a few dedicated minutes when you file so you can submit and immediately check the status, rather than filing and walking away.

**If you miss the window:** Don't worry, and don't resubmit. Your filing was most likely still accepted; the portal is just slow to show it. You can confirm your organization's real, accepted filing status anytime on the IRS's public [Tax Exempt Organization Search](https://apps.irs.gov/app/eos/details/) tool. It will show your e-Postcard filing as accepted even when the [e-Postcard portal](https://sa.www4.irs.gov/epostcard/secure/990n/forms/list/) still shows "Pending."

---

## State Annual Reports and Registration Renewals

Most states require nonprofits to file an annual report with the Secretary of State to keep the organization in good standing. Fees and deadlines vary by state. Missing filings can result in administrative dissolution.

Check your state's Secretary of State website each year.

---

## Board Meeting Minutes

Boards are required to hold regular meetings and keep written minutes on file. Minutes should document:

* Date, location, and attendees
* Decisions and votes taken
* Any conflicts of interest disclosed
* Actions assigned

Minutes do not need to be exhaustive, but they must exist. Store them permanently; they may be requested by auditors, lenders, or state regulators.

---

## Financial Records Retention

Keep the following records for the periods noted:

* **Permanent:** Articles of incorporation, bylaws, IRS determination letter, board meeting minutes, tax returns (990s)
* **7 years:** General financial records, bank statements, grant agreements, contracts
* **3 years:** Non-permanent HR records

The IRS can audit up to 3 years back in most cases, and up to 6 years if it suspects substantial underreporting.

---

## State Charitable Solicitation Registration

If your organization solicits donations (in person, by mail, online, or through third parties), most states require you to register as a charitable solicitor. This is separate from your tax exemption and your annual report.

* Requirements vary widely: some states exempt small organizations, some require registration before your first ask.
* If you fundraise online, you may need to register in multiple states.
* [Charitable Allies](https://charitableallies.org/) and the [National Council of Nonprofits](https://www.councilofnonprofits.org/) both have state-by-state registration guidance.

---

*This page is maintained by [Good Heart Tech](https://goodhearttech.org). We provide free IT services to nonprofits in the Treasure Valley and beyond.*
